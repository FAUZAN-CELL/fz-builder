...
function getAppVersions(isRooted, page = 1) {
  document.getElementsByTagName('header')[0].innerHTML = `
    ...
    ${
      isRooted
        ? '<span><strong>You are building rooted</strong>, Builder will automatically download the correct version for you.<br>If you didn\'t intend on doing a rooted build, include all "Root required to exclude" patches<span>'
        : ''
    }
    `;
...
function installPatch() {
  sendCommand({ event: 'installPatch' });
}
    ...
    case 'fileExists':
      {
        ...
        document.getElementsByTagName('header')[0].innerHTML = `
            ...
            <span>The APK already exists in the patched folder.${
              message.isRooted ? ' ' : '<br>'
    ...
    case 'buildFinished':
      {
        ...
        if (!WS_URI.startsWith('ws://localhost'))
          firstFooterElement.innerHTML +=
            '<button class="highlighted" onclick="window.open(\'/patched.apk\', \'_blank\')">Download</button>';
