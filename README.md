...
This project will allow you to download the APK of any of the [officially supported](https://github.com/FAUZAN-CELL/fz-patches#-patches) apps and build easily!
...
If you are on a PC, download the latest executable from [here](https://github.com/FAUZAN-CELL/fz-builder/releases/latest) or if you are on a Android device, please see [this](https://github.com/FAUZAN-CELL/fz-builder/wiki/How-to-use-builder-on-Android).
...
For developers, see [this](https://github.com/FAUZAN-CELL/fz-builder/blob/main/DEVELOPERS.md)
...
Clone the repository and `cd` into the directory `fz-builder`
...
**Note: docker-compose uses docker-compose.yml so make sure you are in the same directory `fz-builder`**
...
```bash
docker run -d --name <name_of_container> -p 8000:8000 --restart unless-stopped -v ./patched/:/app/fz-builder/patched/ <name_of_the_image>
```
...
In both the builds, a persistent storage is kept. All the builds are stored in `<path/to>/fz-builder/patched/`.
