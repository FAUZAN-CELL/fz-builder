...
const mountPatchInstaller = require('../utils/mountPatchInstaller.js');

module.exports = async function installPatch(ws) {
  if (
    ...
    for (const deviceId of global.jarNames.devices) {
      ws.send(
        JSON.stringify({
          event: 'patchLog',
          log: `INFO: Installing Patch (non-root) for ${deviceId}.`
        })
      );
      try {
        await exec(
        ...
        ws.send(
          JSON.stringify({
            event: 'patchLog',
            log: `INFO: Installed Patch (non-root) for ${deviceId}.`
          })
        );
      } catch (e) {
        ws.send(
          JSON.stringify({
            event: 'patchLog',
            log: `SEVERE: Could not install Patch to ${deviceId}. Please check the console for more info.`
          })
        );
    ...
    for (const deviceId of global.jarNames.devices) {
      ws.send(
        JSON.stringify({
          event: 'patchLog',
          log: `INFO: Installing Patch (root) for ${deviceId}.`
        })
      );
      try {
        mountPatchInstaller(deviceId);

        ws.send(
          JSON.stringify({
            event: 'patchLog',
            log: `INFO: Installed Patch (root) for ${deviceId}.`
          })
        );
      } catch (e) {
        ws.send(
  ...
  if (
    ...
    const currentMicroGVersion = (
      await getDownloadLink({ owner: 'FAUZAN-CELL', repo: 'MicroG' })
    ).version
      .replace('v', '')
      .split('-')[0];
