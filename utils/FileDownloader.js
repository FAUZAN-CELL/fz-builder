...
async function overWriteJarNames(fileName) {
  const filePath = joinPath(global.patchedDir, fileName);
  if (fileName.includes('fz-cli')) global.jarNames.cli = filePath;

  if (fileName.includes('fz-patches') && fileName.endsWith('.jar'))
...
async function downloadFile(assets) {
  for (const asset of assets.assets) {
    const dir = readdirSync(global.patchedDir);
    ...
    await dloadFromURL(
      ...
      joinPath(global.patchedDir, fileName)
    );
  }
}
