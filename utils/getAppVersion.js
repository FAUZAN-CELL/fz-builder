...
function sendError(ws) {
  ws.send(
    JSON.stringify({
      event: 'error',
      error:
        "The app you selected is not installed on your device. It's needed for rooted patched."
    })
  );
}
...
async function getAppVersion(pkgName, ws, shouldReturnMsg) {
  ...
  if (versions.every((version) => version === versions[0])) {
    ...
    return ws.send(
      JSON.stringify({
        event: 'error',
        error: `The devices you're trying to install doesn't have a matching version. Please install version ${versions[0]} to every device.`
      })
    );
  }
}
